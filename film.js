...
document.addEventListener('DOMContentLoaded', () => {
    ...
    noteForm.addEventListener('submit', (event) => {
        event.preventDefault(); // Zamezí výchozímu odesílání formuláře

        // Ověření, zda byl text napsán
        ...
        // Ověření, zda byly akceptovány podmínky
        ...
        // Aktualizace HTML obsahu, pokud je formulář platný
        if (isFormValid) {
...
document.addEventListener('DOMContentLoaded', () => {
    const filmId = window.location.hash.substring(1); // Odstraní mřížku (#) z hash
    ...
    if (film) {
        ...
        document.querySelector('.img-fluid').alt = `Plakát filmu ${film.nazev}`;
    }

    // Zde bude zbytek vašeho kódu pro ošetření událostí formuláře a hvězdiček hodnocení
});